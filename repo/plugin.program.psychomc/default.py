...
base='bit.ly/PsychoFacebook'
ADDON=xbmcaddon.Addon(id='plugin.program.psychowiz')
...
def CATEGORIES():
	...
	link = OPEN_URL('http://psycho.pe.hu/file5.php?user='+USER+'&pass='+PASS).replace('\n','').replace('\r','')
...
def wizard(name,url,description):
    ...
    dp.create("[COLOR aqua]P[/COLOR][COLOR white]sycho Wizard[/COLOR]","A fazer download ",'', 'Espere por favor')
    ...
    time.sleep(2)
    dp.update(0,"", "Extraindo")
    print '======================================='
    print addonfolder
    print '======================================='
    extract.all(lib,addonfolder,dp)
    dialog = xbmcgui.Dialog()
    dialog.ok("DOWNLOAD COMPLETO", 'Unfortunately the only way to get the new changes to stick is', 'to force close kodi. Click ok to force Kodi to close,', 'DO NOT use the quit/exit options in Kodi., If the Force close does not close for some reason please Restart Device or kill task manaully')
    killxbmc()
